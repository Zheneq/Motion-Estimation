                          ...
                          const bool use_half_pixel)
{
	...
	for (int i = 0; i < num_blocks_vert; i++)
	{
		for (int j = 0; j < num_blocks_hor; j++)
		{
            ...
            // Split the block in 4 blocks if the error it too large
			if (prob_motion_vector.error > SPLIT_TRESH)
			{
				...
				for (int h = 0; h < 4; h++)
				{
					...
					for (int k = -mm; k < mm; k++)
					{
						for (int l = -mm; l < mm; l++)
						{
							error = GetErrorSAD_8x8(cur, prev + k * wext + l, wext);
							if (error < min_error)
							{
								prob_motion_vector.x = l;
								prob_motion_vector.y = k;
                ...
                if (MVectors[block_id].SubVector(0).error +
                    MVectors[block_id].SubVector(1).error +
                    MVectors[block_id].SubVector(2).error +
                    MVectors[block_id].SubVector(3).error >
					MVectors[block_id].error * 0.7)
				{
					MVectors[block_id].Unsplit();
				}
			}
		}
	}
